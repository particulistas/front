                    ...
                    <main class="max-w-6xl mx-auto px-4 py-4">
                        <div class="flex justify-between items-center px-4 py-2 bg-white">
                            ...
                            <span class="text-sm md:hidden">
                                <strong>{{ property.category?.name }} en {{ transactionText }}</strong>
                            </span>
                            ...
                            <div class="md:w-1/2 bg-white rounded-lg shadow-sm p-4">
                                ...
                                <div class="bg-blue-400 h-64 rounded-lg mb-2 relative">
                                    <img 
                                        ...
                                        :src="imageUrl + property.media[0].path" 
                                ...
                                <div class="flex space-x-2 mb-4 overflow-x-auto pb-2">
                                    <template v-if="property.media && property.media.length > 0">
                                        <div 
                                            v-for="(media, index) in property.media.slice(0, NumberThumbnails)" 
                                            :key="media.id"
                                            @click="changeMainImage(index)"
                                            class="w-16 h-16 bg-gray-200 rounded cursor-pointer flex-shrink-0 overflow-hidden"
                                        >
                                            <img 
                                                :src="imageUrl + media.path" 
                                                class="w-full h-full object-cover"
                                                :alt="'Imagen ' + (index + 1) + ' de la propiedad'"
                                            >
                                        </div>
                                        <div 
                                            v-if="property.media.length > NumberThumbnails"
                                            @click="changeNumberThumbnails"
                                            class="w-16 h-16 bg-white border border-gray-200 rounded flex items-center justify-center text-xs flex-shrink-0 cursor-pointer hover:bg-gray-50 transition-colors"
                                            >
                                            +{{ property.media.length - NumberThumbnails }} Fotos
                                        </div>
                                    </template>
                                    <template v-else>
                                        <!-- Placeholders mientras carga o si no hay imágenes -->
                                        <div v-for="i in 4" :key="'placeholder-'+i" class="w-16 h-16 bg-gray-200 rounded flex-shrink-0"></div>
                                    </template>
                                </div>
                        ...
                        <div class="md:w-1/2 left-0 right-0 bg-white  p-4 flex gap-4 mt-6">
                            <button @click="publish" class="flex-1  text-white py-3 px-4 rounded-lg border-2 border-[#27ABB1] icon-container-hover hover:bg-[#27ABB1] group shadow-par">
...
<script setup>
    ...
    import ConfirmationModal from './components/Publish/ConfirmationModal.vue'
    ...
    const property = ref({
        media: [] // Inicializar como array vacío
    });
    ...
    const imageUrl = ref('');
    const NumberThumbnails = ref(4); // Valor inicial 4
    const latitude = ref(null);
    const longitude = ref(null);
    const mapContainer = ref(null);

    const changeMainImage = (index) => {
        if (!property.value.media || index >= property.value.media.length) return;
            const selectedImage = property.value.media[index];
            const newMediaArray = [
                selectedImage,
                ...property.value.media.slice(0, index),
                ...property.value.media.slice(index + 1),
        ];
        
        property.value.media = newMediaArray;
    };

   
    const changeNumberThumbnails = () => {
        NumberThumbnails.value = property.value.media.length; // Cambia el valor a 8 cuando se hace clic
    };
    ...
    async function getProperties(id) {
        try {
            ...
            // Asegurarse de que media sea siempre un array
            property.value = {
                ...data,
                media: data.media || [] // Si data.media es undefined, usar array vacío
            };
            ...
            property.value.media = []; // En caso de error, asegurar array vacío
        }
    };
